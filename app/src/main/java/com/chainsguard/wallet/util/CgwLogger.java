 ...
 */
public class CgwLogger {
    private static final String MAIN_TAG = "CGW";
    ...
    private static Set<String> sTagCloseFilter = new HashSet<>();

    public static void addCloseClass(Class<?> clazz) {
        sTagCloseFilter.add(clazz.getSimpleName());
    }
    ...
    private static int getStackDepth(StackTraceElement[] elements) {
        for (int i = STACK_DEPTH; i < elements.length; i++) {
            ...
            if (!element.getClassName().split("\\$")[0].equals(CgwLogger.class.getName())) {
                return i;
            }
        }
    ...
    public static void i(String tag, String format, Object... args) {
        if (PRINT) {
            if (sTagCloseFilter.contains(tag)) { return; }
